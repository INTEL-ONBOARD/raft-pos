...
export function useSalesByProduct(filters: ReportFilters | null) {
  return useQuery({
    ...
    queryFn: async () => {
      const result = await ipc.invoke<SalesByProductResult>(
        IPC.REPORTING_GET_SALES_BY_PRODUCT,
        filters
      )
      if (!result.success) throw new Error(result.error)
      return result
...
export function useInventoryValuation(filters: ReportFilters | null) {
  return useQuery({
    ...
    queryFn: async () => {
      const result = await ipc.invoke<InventoryValuationResult>(
        IPC.REPORTING_GET_INVENTORY_VALUATION,
        filters
      )
      if (!result.success) throw new Error(result.error)
      return result
...
export function useCashDrawerReport(filters: ReportFilters | null) {
  return useQuery({
    ...
    queryFn: async () => {
      const result = await ipc.invoke<CashDrawerReportResult>(
        IPC.REPORTING_GET_CASH_DRAWER_REPORT,
        filters
      )
      if (!result.success) throw new Error(result.error)
      return result
