...
export function BranchFormModal({ branch, onConfirm, onClose, isLoading }: Props) {
  ...
  function handleCodeChange(v: string) {
    setCode(v.toUpperCase().replace(/[^A-Z0-9\-]/g, '').slice(0, 10))
    setCodeTouched(true)
  }
          ...
          <div className="px-6 py-5 space-y-3">
            <div>
              ...
              <input value={name} onChange={e => handleNameChange(e.target.value)}
                placeholder="e.g. Main Branch" className="dark-input mt-1" />
            </div>
            <div>
              <label style={labelStyle}>Code *</label>
              <input value={code} onChange={e => handleCodeChange(e.target.value)}
                placeholder="e.g. MAIN" className="dark-input mt-1 font-mono" />
            ...
            <div>
              ...
              <input value={address} onChange={e => setAddress(e.target.value)}
                placeholder="e.g. 123 Main St" className="dark-input mt-1" />
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
              <div>
                <label style={labelStyle}>Phone</label>
                <input value={phone} onChange={e => setPhone(e.target.value)}
                  placeholder="+63 9XX XXX XXXX" className="dark-input mt-1" />
              </div>
              <div>
                <label style={labelStyle}>Email</label>
                <input type="email" value={email} onChange={e => setEmail(e.target.value)}
                  placeholder="branch@example.com" className="dark-input mt-1" />
              </div>
            </div>
            {error && (
