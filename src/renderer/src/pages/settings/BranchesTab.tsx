...
export function BranchesTab() {
  ...
  async function handleUpdate(data: any) {
    ...
    try { await updateMutation.mutateAsync({ id: editBranch._id, input: data }); setEditBranch(null) }
    catch (err: any) { setError(err.message) }
  }

  async function handleDeactivate(b: IBranch) {
    if (!confirm(`Deactivate "${b.name}"? Users assigned to this branch will still exist but the branch will be hidden from new assignments.`)) return
    try { await deactivateMutation.mutateAsync(b._id) }
    catch (err: any) { setError(err.message) }
  }
    ...
    <>
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '16px' }}>
        <button onClick={() => { setShowCreate(true); setError('') }} className="btn-primary"
          style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 18px', fontSize: '14px', fontWeight: 600 }}>
      ...
      {error && (
        <div style={{
          ...
          padding: '14px', fontSize: '13px', borderRadius: '12px',
      ...
      <div style={{
        ...
        {branchesQuery.isError ? (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '64px 0', gap: '12px' }}>
            ...
            <button onClick={() => branchesQuery.refetch()}
                  ...
                  {Array.from({ length: 5 }).map((__, j) => (
                    <td key={j} style={{ padding: '12px 16px', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                      <div className="animate-pulse" style={{
                        height: '13px', borderRadius: '6px', background: 'rgba(255,255,255,0.06)',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '64px 0', gap: '12px' }}>
            <div style={{
              ...
              <BuildingOffice2Icon style={{ width: '22px', height: '22px', color: 'rgba(255,255,255,0.28)' }} />
            </div>
            <div style={{ textAlign: 'center' }}>
              <p style={{ fontSize: '14px', color: 'rgba(255,255,255,0.55)', margin: 0 }}>No branches yet</p>
              <p style={{ fontSize: '12px', color: 'rgba(255,255,255,0.28)', marginTop: '4px' }}>Add your first branch to get started.</p>
            </div>
          </div>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              {branches.map((b, idx) => {
                ...
                return (
                  <tr key={b._id}
                    ...
                    style={{ height: '56px', background: hoveredRow === b._id ? 'rgba(255,255,255,0.03)' : 'transparent' }}>
                    <td style={{ padding: '12px 16px', borderBottom: isLast ? 'none' : '1px solid rgba(255,255,255,0.05)', fontSize: '13px' }}>
                      <span style={{ color: 'rgba(255,255,255,0.88)', fontWeight: 500 }}>{b.name}</span>
                    </td>
                    <td style={{ padding: '12px 16px', borderBottom: isLast ? 'none' : '1px solid rgba(255,255,255,0.05)', fontSize: '13px' }}>
                      <span style={{
                    ...
                    <td style={{ padding: '12px 16px', borderBottom: isLast ? 'none' : '1px solid rgba(255,255,255,0.05)', fontSize: '13px', color: 'rgba(255,255,255,0.50)' }}>
                      {b.address || '—'}
                    </td>
                    <td style={{ padding: '12px 16px', borderBottom: isLast ? 'none' : '1px solid rgba(255,255,255,0.05)' }}>
                      {b.isActive
                    ...
                    <td style={{ padding: '12px 16px', borderBottom: isLast ? 'none' : '1px solid rgba(255,255,255,0.05)' }}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: '4px' }}>
                        <button onClick={() => { setEditBranch(b); setError('') }}
                          style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '32px', height: '32px', borderRadius: '8px', border: 'none', cursor: 'pointer', color: 'rgba(255,255,255,0.35)', background: 'transparent' }}
