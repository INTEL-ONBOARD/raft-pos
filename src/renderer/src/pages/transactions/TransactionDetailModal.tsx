    ...
    <div className="modal-overlay fixed inset-0 z-50 flex items-center justify-center p-4">
      <div className="modal-panel w-full max-w-xl max-h-[90vh] overflow-y-auto overflow-hidden">
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          {/* Totals */}
          <div
            className="pt-3 space-y-1 text-sm"
          ...
          >
            ...
            {t.discountAmount > 0 && (
              <div className="flex justify-between" style={{ color: 'var(--text-secondary)' }}>
                <span>Discount</span>
                <span>-₱{fmt(t.discountAmount)}</span>
              </div>
            )}
            {t.taxAmount > 0 && (
              <div className="flex justify-between" style={{ color: 'var(--text-secondary)' }}>
                <span>Tax ({t.taxRate}%)</span>
                <span>₱{fmt(t.taxAmount)}</span>
              </div>
            )}
            <div
          ...
          {t.status === 'voided' && t.voidReason && (
            ...
            >
              ...
              {t.voidedAt && (
                <span className="block text-xs mt-0.5" style={{ color: 'rgba(220,38,38,0.65)' }}>
                  {new Date(t.voidedAt).toLocaleString()}
                </span>
              )}
            </div>
          )}
          {(t.status === 'refunded' || t.status === 'partially_refunded') && t.refundReason && (
            <div
              className="px-4 py-3 text-sm rounded-lg"
              style={{
            ...
            >
              ...
              {t.refundedAt && (
                <span className="block text-xs mt-0.5" style={{ color: 'rgba(180,83,9,0.65)' }}>
                  {new Date(t.refundedAt).toLocaleString()}
                </span>
              )}
            </div>
          )}
