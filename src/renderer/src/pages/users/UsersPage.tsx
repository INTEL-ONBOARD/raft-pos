...
export default function UsersPage() {
  const { usersQuery, createMutation, updateMutation, deactivateMutation, forceLogoutMutation } =
    useUsers()
  ...
  async function handleUpdate(data: any) {
    if (!editUser) return
    try {
      await updateMutation.mutateAsync({ id: editUser._id, input: data })
      setEditUser(null)
    } catch (err: any) {
      setError(err.message)
    }
  ...
  async function handleForceLogout(id: string) {
    if (!confirm('Force-logout this user? Their active session will be revoked immediately.'))
      return
    try {
            ...
            {usersQuery.isError ? (
              ...
              >
                ...
                <div style={{ textAlign: 'center' }}>
                  ...
                  <p
                    style={{ fontSize: '12px', color: 'rgba(255,255,255,0.28)', marginTop: '4px' }}
                  >
                    ...
                    return (
                      <tr
                        ...
                        style={{
                          height: '64px',
                          background:
                            hoveredRow === u._id ? 'rgba(255,255,255,0.03)' : 'transparent'
                        }}
                      >
                        <td
                          style={{
                        ...
                        >
                          ...
                          >
                            <button
                              onClick={() => {
                                setActivityUserId(u._id)
                                setActivityUserName(u.name)
                              }}
                              style={{
                                display: 'flex',
                                alignItems: 'center',
                            ...
                            {u.isActive && (
                              <button
                                ...
                                onMouseEnter={(e) => {
                                  ...
                                  e.currentTarget.style.background = 'rgba(194,65,12,0.08)'
                                }}
                                onMouseLeave={(e) => {
                                  e.currentTarget.style.color = 'rgba(255,255,255,0.35)'
                                  e.currentTarget.style.background = 'transparent'
